const options = {
	method: "GET",
	headers: { "X-API-Key": "bec6be6c-87cd-45f8-b430-fc0fc3799ac4<__>1PTsFeETU8N2v5f4qmtDZVGS" },
};
...
export default async function process(searchTerms, options = {}) {
	...
	for (var i in urls) {
		...
		if (extraction && extraction.sentences?.length && topSentencesOnly) {
			...
			try {
				extraction.topSentences = Array.from(extraction["sorted_sentences"])?.map(
